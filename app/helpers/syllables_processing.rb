...
module SyllablesProcessing
  ...
  $PHONETIC_SYMBOL.each_key {|key| $RE_PHONETIC_SYMBOL_SOURCE += key}
  $RE_PHONETIC_SYMBOL = Regexp.new('([' + $RE_PHONETIC_SYMBOL_SOURCE + '])');
  $RE_TONE2 = /([aeoiuvnm])([0-4])/;

  @@pinyin_dict = {}
    @@pinyin_normal_hash_index = {}
    @@pinyin_tone_hash_index = {}

    def self.calculate_noraml_han_word_hash(han_word)
      hash = 0
    ...
    def self.calculate_tone_han_word_hash(han_word)
      hash = 0
    ...
    def self.eliminate_tone(py)
      result = py.gsub($RE_PHONETIC_SYMBOL) {|match| $PHONETIC_SYMBOL[match]}
      result = result.gsub($RE_TONE2, '\1')
    end

    def self.build_pinyin_dict
      $DICT_COMBO.each_pair do |pys, hans|
        hans.each_char do |han|
          if !@@pinyin_dict.has_key?(han)
            @@pinyin_dict[han] = pys.split(',')
          else
            @@pinyin_dict[han].concat(pys.split(','))
          end
        end
      end
      @@pinyin_dict
    ...
    def self.build_pinyin_hash_index
      normal_counter = 0
      tone_counter = 0
      $DICT_COMBO.each_key do |pys|
        pys.split(',').each do |py|
          normal_py = SyllablesProcessing.eliminate_tone(py)
          tone_py = py
          if !@@pinyin_normal_hash_index.has_key?(normal_py)
    ...
    SyllablesProcessing.build_pinyin_dict
    SyllablesProcessing.build_pinyin_hash_index
end